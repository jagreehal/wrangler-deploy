...
Need a public URL for a webhook or OAuth callback? Pass `--tunnel` to spawn
...
in front of one or all running workers:
...
Starting dev servers:
  workers/api -> http://localhost:8787
  tunnel: workers/api -> https://round-fox-42.trycloudflare.com
```

Tunnels shut down with the workers on Ctrl+C. Requires `cloudflared` on your
PATH — install via Cloudflare's [downloads page](https://developers.cloudflare.com/cloudflare-one/connections/connect-networks/downloads/).
