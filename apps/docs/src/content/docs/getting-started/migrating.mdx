...
export default defineConfig({
  ...
  resources: {
    ...
    "cache-kv": {
      type: "kv",
      bindings: { "workers/api": "CACHE" },
    },
    "outbox": {
      type: "queue",
      bindings: {
        "workers/api": { producer: "OUTBOX" },
        "workers/batch": { consumer: true },
      },
    },
  },
});
...
Workers are paths to directories that contain a `wrangler.jsonc`. Resource keys are logical names; the actual Cloudflare resource gets the stage suffix at apply time (e.g. `payments-db-staging`).

## Step 3: Protect production

Before doing anything else, add stage protection so `wd destroy --stage production` refuses to run without `--force`. The `stages` field is a map keyed by stage name; wildcards like `pr-*` match every matching stage.
...
`ttl` is honoured by `wd gc`, which destroys expired unprotected stages.
