...
const stageA: StageState = {
  ...
  resources: {
    "payments-db": {
      type: "d1",
      desired: { name: "staging-payments-db" },
      observed: { id: "db-staging-001", status: "active" },
      source: "managed",
    },
    "cache-kv": {
      type: "kv",
      desired: { name: "staging-cache-kv" },
      observed: { id: "kv-staging-001", status: "active" },
      source: "managed",
    },
    "staging-only-kv": {
      type: "kv",
      desired: { name: "staging-only-kv" },
      observed: { id: "kv-staging-extra", status: "active" },
      source: "managed",
    },
  },
  workers: {
...
const stageB: StageState = {
  ...
  resources: {
    "payments-db": {
      type: "d1",
      desired: { name: "prod-payments-db" },
      observed: { id: "db-prod-001", status: "active" },
      source: "managed",
    },
    "cache-kv": {
      type: "kv",
      desired: { name: "prod-cache-kv" },
      observed: { id: "kv-prod-001", status: "active" },
      source: "managed",
    },
  },
  workers: {
  ...
  it("classifies a resource only in B as only-in-b", ({ task }) => {
    ...
    const stateWithExtraB: StageState = {
      ...
      resources: {
        ...
        "prod-only-queue": {
          type: "queue",
          desired: { name: "prod-only-queue" },
          observed: { id: "q-prod-extra", status: "active" },
          source: "managed",
        },
      },
    };
  ...
  it("classifies a resource with different type as different", ({ task }) => {
    ...
    const stateWithDifferentType: StageState = {
      ...
      resources: {
        ...
        "cache-kv": {
          type: "d1",
          desired: { name: "prod-cache-kv-d1" },
          observed: { id: "d1-prod-001", status: "active" },
          source: "managed",
        },
      },
    };
