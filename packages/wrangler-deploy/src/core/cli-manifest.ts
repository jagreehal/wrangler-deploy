...
export const cliManifest = {
  ...
  commands: [
    ...
    { name: "init", description: "Generate wrangler-deploy.config.ts from local Wrangler configs.", flags: ["--json", "--preset", "--force", "--dry-run"], mutating: true, writesFiles: true, supportsDryRun: true, output: "text" },
    ...
    { name: "doctor", description: "Validate wrangler and repo prerequisites.", flags: ["--json", "--codes", "--fix", "--fix-dry-run", "--strict"], supportsDryRun: true, output: "json" },
    ...
    { name: "schema", description: "Print the CLI manifest, output schemas, config schema, or error schema.", subcommands: ["outputs", "config", "errors"], flags: ["--json", "--versioned", "--command"], output: "json" },
    ...
    { name: "d1", description: "List, inspect, and execute local D1 workflows.", subcommands: ["list", "inspect", "exec", "seed", "reset", "migrate"], flags: ["--json", "--dry-run"], mutating: true, writesFiles: true, supportsDryRun: true, output: "json" },
    ...
    { name: "dev", description: "Start local dev servers with auto port resolution. Long-running. With --json/--ndjson, emits one event per line (see `wd schema outputs --command devEvent`).", subcommands: ["doctor", "ui"], flags: ["--filter", "--session", "--tunnel", "--base-port", "--fallback-stage", "--persist-to", "--port", "--json", "--ndjson"], network: true, output: "json" },
